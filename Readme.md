...
Python实现了描述符功能，正是基于这个功能实现了新式类（new-style class）的对象模型
...
**这个绝对常考啊.绝对要记住`1~2个方法`,当时面试官是让手写的.** **单例模式天天问怎么实现`线程安全`？怎么提高效率**
...
![img](1418490-20180906153626089-1835444372.png)
...
在一个外函数中定义了一个内函数，内函数里运用了外函数的临时变量，并且外函数的返回值是内函数句柄的引用
...
### 1 引用计数 $\Rightarrow $ 对每个对象维护一个引用计数，当引用计数器为零时回收该对象
...
### 3 分代技术 **Generational Collection**
